...
public class Kangaroo extends Animal {
    ...
    private static final int maxCountThisAnimalTypeToCell = 149;
    private final int kidMinCount = changeableClass.getKangarooKidMinCount();
    private final int kidMaxCount = changeableClass.getKangarooKidMaxCount();
    // Скорость перемещения. На сколько клеток можно передвинуться за один такт
    private static final int speed = 2;
    // Мах уровень сытости/насыщения
    private static final double maxSatiety = 7;
    ...
    private final int countThisAnimalTypeToCell = cell.kangaroosListToCell.size();
    ...
    public void born(int locationLength, int locationWidth) {
        ...
        cells[locationLength][locationWidth].kangaroosListToCell.add(new Kangaroo());
    }

    @Override
    public void die(int locationLength, int locationWidth) {
        // то просто уменьшаем на 1
        cells[locationLength][locationWidth].kangaroosListToCell.remove(this);
    }
