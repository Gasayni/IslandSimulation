...
public class Larva extends Animal {
    // Гусеница
    ...
    private final int kidMinCount = changeableClass.getLarvaKidMinCount();
    private final int kidMaxCount = changeableClass.getLarvaKidMaxCount();
    // Скорость перемещения. На сколько клеток можно передвинуться за один такт
    private static final int speed = 1;
    // Мах уровень сытости/насыщения
    private static final double maxSatiety = 0.0025;
    ...
    private static final int maxHungryTactBeforeDie = 1;
    // текущая шкала выживаемости после падения шкалы сытости до нуля
    private int hungryTactBeforeDie = 0;
    private final int countThisAnimalTypeToCell = cell.larvaListToCell.size();
    ...
    public void born(int locationLength, int locationWidth) {
        ...
        cells[locationLength][locationWidth].larvaListToCell.add(new Larva());
    }

    @Override
    public void die(int locationLength, int locationWidth) {
        // то просто уменьшаем на 1
        cells[locationLength][locationWidth].larvaListToCell.remove(this);
    }
