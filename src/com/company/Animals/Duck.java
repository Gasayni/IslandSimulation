...
public class Duck extends Animal {
    ...
    private static final int maxCountThisAnimalTypeToCell = 500;
    private final int kidMinCount = changeableClass.getDuckKidMinCount();
    private final int kidMaxCount = changeableClass.getDuckKidMaxCount();
    // Скорость перемещения. На сколько клеток можно передвинуться за один такт
    private static final int speed = 1;
    // Мах уровень сытости/насыщения
    private static final double maxSatiety = 0.15;
    ...
    private final int countThisAnimalTypeToCell = cell.ducksListToCell.size();
    ...
    public void eat() {
        ...
        // Проверка
//        System.out.println("satietyDuck = " + satiety);
    ...
    public void born(int locationLength, int locationWidth) {
        ...
        cells[locationLength][locationWidth].ducksListToCell.add(new Duck());
    }

    @Override
    public void die(int locationLength, int locationWidth) {
        // то просто уменьшаем на 1
        cells[locationLength][locationWidth].ducksListToCell.remove(this);
    }
