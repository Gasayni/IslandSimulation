...
public class Kangaroo extends Herbivore {
    ...
    // Вес
    private final double weight = 47;
    // мах кол-во животных на одной клетке
    private final int maxCountThisAnimalTypeToCell = 149;
    ...
    @Override
    public void eat() {
        int countThisAnimalTypeToCell = currentCellMap.get("Кенгуру");
        ...
        if (deadFlag || (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell)) {
            die(locationLength, locationWidth, "Кенгуру");
            return;
        }
    ...
    public void reproduction() {
        ...
        if (currentCellMap.get("Кенгуру") > 1) {
            ...
            if (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell) {
                ...
                for (int i = 0; i < details.randomizer(kidMinCount, kidMaxCount); i++) {
                    new Kangaroo().firstBorn(locationLength, locationWidth);
                }
                return;
            }
        }
        ...
        move(locationLength, locationWidth, speed, "Кенгуру");
    }
