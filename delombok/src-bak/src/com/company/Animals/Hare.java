...
public class Hare extends Herbivore {
    ...
    private final int maxCountThisAnimalTypeToCell = 750;
    ...
    private final int maxHungryTactBeforeDie = 7;
    // текущая шкала выживаемости после падения шкалы сытости до нуля
    private int hungryTactBeforeDie = 0;

    @Override
    public void eat() {
        int countThisAnimalTypeToCell = currentCellMap.get("Заяц");
        ...
        if (deadFlag || (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell)) {
            die(locationLength, locationWidth, "Заяц");
            return;
        }
    ...
    public void reproduction() {
        ...
        if (currentCellMap.get("Заяц") > 1) {
            ...
            if (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell) {
                ...
                for (int i = 0; i < details.randomizer(kidMinCount, kidMaxCount); i++) {
                    new Hare().firstBorn(locationLength, locationWidth);
                }
                return;
            }
        }
        ...
        move(locationLength, locationWidth, speed, "Заяц");
    }
