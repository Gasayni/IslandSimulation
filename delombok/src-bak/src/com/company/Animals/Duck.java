...
public class Duck extends Animal {
    ...
    private final int maxCountThisAnimalTypeToCell = 500;
    ...
    private final int maxHungryTactBeforeDie = 4;
    // текущая шкала выживаемости после падения шкалы сытости до нуля
    private int hungryTactBeforeDie = 0;

    @Override
    public void eat() {
        int countThisAnimalTypeToCell = currentCellMap.get("Утка");
        ...
        if (deadFlag || (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell)) {
            die(locationLength, locationWidth, "Утка");
            return;
        }
    ...
    public void reproduction() {
        ...
        if (currentCellMap.get("Утка") > 1) {
            ...
            if (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell) {
                ...
                for (int i = 0; i < details.randomizer(kidMinCount, kidMaxCount); i++) {
                    new Duck().firstBorn(locationLength, locationWidth);
                }
                return;
            }
        }
        ...
        move(locationLength, locationWidth, speed, "Утка");
    }
