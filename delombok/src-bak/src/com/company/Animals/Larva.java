...
public class Larva extends Herbivore {
    ...
    private final double maxSatiety = 0.0025;
    ...
    private final int maxHungryTactBeforeDie = 1;
    // текущая шкала выживаемости после падения шкалы сытости до нуля
    private int hungryTactBeforeDie = 0;

    @Override
    public void eat() {
        int countThisAnimalTypeToCell = currentCellMap.get("Гусеница");
        ...
        if (deadFlag || (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell)) {
            die(locationLength, locationWidth, "Гусеница");
            return;
        }
    ...
    public void reproduction() {
        ...
        if (currentCellMap.get("Гусеница") > 1) {
            ...
            if (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell) {
                ...
                for (int i = 0; i < details.randomizer(kidMinCount, kidMaxCount); i++) {
                    new Larva().firstBorn(locationLength, locationWidth);
                }
                return;
            }
        }
        ...
        move(locationLength, locationWidth, speed, "Гусеница");
    }
