...
public class Hamster extends Herbivore {
    ...
    // Вес
    private final double weight = 0.03;
    // мах кол-во животных на одной клетке
    private final int maxCountThisAnimalTypeToCell = 10000;
    private final int kidMinCount = changeableClass.getHamsterKidMinCount();
    private final int kidMaxCount = changeableClass.getHamsterKidMaxCount();
    // Скорость перемещения. На сколько клеток можно передвинуться за один такт
    private final int speed = 1;
    // Мах уровень сытости/насыщения
    private final double maxSatiety = 0.0075;
    ...
    private final int maxHungryTactBeforeDie = 3;
    // текущая шкала выживаемости после падения шкалы сытости до нуля
    private int hungryTactBeforeDie = 0;

    @Override
    public void eat() {
        int countThisAnimalTypeToCell = currentCellMap.get("Хомяк");
        ...
        if (deadFlag || (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell)) {
            die(locationLength, locationWidth, "Хомяк");
            return;
        }
    ...
    public void reproduction() {
        ...
        if (currentCellMap.get("Хомяк") > 1) {
            ...
            if (countThisAnimalTypeToCell < maxCountThisAnimalTypeToCell) {
                ...
                for (int i = 0; i < details.randomizer(kidMinCount, kidMaxCount); i++) {
                    new Hamster().firstBorn(locationLength, locationWidth);
                }
                return;
            }
        }
        ...
        move(locationLength, locationWidth, speed, "Хомяк");
    }
